...
build_project() {
    printf " === Building project with npm...\n"
    npm ci 
    if [ "$NPM_AUDIT_FORCE_FIX" = "TRUE" ]; then
        printf " === Running npm audit fix with --force...\n"
        npm audit fix --force
    else
...
if [ -f /usr/local/apache2/htdocs/package.json ]; then
    ...
    printf " === 5eTools src should now be up to date and ready to use.\n"
else 
    VERSION="unknown (no package.json)"
    printf " === No package.json found, cannot determine version. Exiting.\n"
    exit 1
fi
